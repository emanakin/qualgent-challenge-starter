...
# Write convenient env exports for downstream scripts
ENV_FILE="$STATE_DIR/env.sh"
HP_FIRST="$(awk 'NR==1{print $2}' "$ADB_FILE" | tr -d '",')"
if [ -n "$HP_FIRST" ]; then
  {
    echo "# Generated by create_devices.sh - source this file to set ADB env vars"
    echo "export ADB_HOSTPORT=\"$HP_FIRST\""
    echo "export ANDROID_SERIAL=\"$HP_FIRST\""
  } > "$ENV_FILE"
  echo "[infra] Wrote env exports to $ENV_FILE"
  echo "[infra] Tip: source $ENV_FILE && adb connect \$ADB_HOSTPORT && adb devices"
fi